In this chapter the kinematics are presented, which will follow us later for the calculation of the splitting functions constantly. In addition, the most relevant formula will be derived and a strategy for simplifying the calculations is going to be defined.  
...
The variable $ y_{ij,k} = \frac{q_i \cdot q}{p_i \cdot p_j}$ need to be introduced, that is zero in the soft and collinear limit. Note that both the emitter and the spectator are on-shell. From the conservation of momentum derives:
...
\begin{equation}
    ...
    \end{split}
\end{equation}
$ k = 1,...,n $ labels the emission momenta and is taken to be massless $ {k_l}^2 = 0 $. Where the label $ l $ denotes the count of emissions. In this work just the one-emission kernels are considered. All hard momenta in this mapping are also on-shell, $ {p_k}^2={q_k}^2=0 $.
...
\end{split}	
\end{equation}
According to the light-cone coordinates of the momenta, the following condition must be taken into account:

\begin{equation}
\begin{split}
	{n^{\mu}}_{\bot,l}{\Lambda_{\mu}}^{\nu}{p_i}_{\nu} &= {n_{\bot,l}} \cdot n = {n_{\bot,l}} \cdot Q =0\\
	{n^{\mu}}_{\bot,l}\cdot p_k &\neq0\\
\end{split}	
\end{equation}

$ {n}^2_{\bot,l} = -2\alpha{\Lambda^{\mu}}_{\nu}{p_i}^{\nu} n_{\mu} $ is not on-shell and in terms of the single emission case it follows $ {n}^2_{\bot,1} = -2p_i\cdot Q $.
...
From the parametrisation it is necessary to calculate the respective Lorenz transformation for the Emitters, Spectator and total momentum first. The general definition of the transformation is given by:
...
In terms of one emission where $ l=1 $ the mapping can be simplified to:
\begin{equation}
	\begin{aligned}
...
To investigate the mapping it is useful to determine first the dot products between these four vectors. The often occurring pre-factor products are given in \ref{pre}.
...
\label{recepie}
As mentioned in the last chapter the singularities come from the propagators in each diagram since the denominators according to Feynmann rules contain terms with $\sim \frac{1}{2 q_a \cdot q_b}  $. Whereby a and b label the respective momenta. Since the calculations are sometimes very complicated and confusing, the procedure for eliminating the finite terms is as follows:\\
When calculating the square matrix elements, products in the form of $ p_a \cdot p_b $ always appear both in the numerator and denominator.
The denominator shows which pre-factor causes the singularity. These terms from the numerator with the same prefix can be omitted from the beginning and are therefore finite. This is explicitly shown below for the common denominators. This operation can also be performed for the other types of the denominator.
