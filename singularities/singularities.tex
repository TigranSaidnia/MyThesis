...
The symbol $ \otimes $ is for phase space convolutions and sums over colour and spin indices. $ \partial V_{dipoles} $ will compute just once for all and match the singular property of the real part. They are universal in the sense that they do not depend on the hard scattering. The good news is that we are now allowed to use a factorizable mapping from
...
is absorbed in the parton distribution. Partons with larger transverse momenta participate in the hard scattering process with a short-distance partonic cross-section.
The factorisation theorem also applies to deep inelastic scattering. It can be shown how the IR singularities are absorbed in the PDFs. The DIS cross section can be written as ~\cite{Ellis:1991qj}
\begin{equation}
...
fraction between $ x $ and $ \partial x $ of the proton momentum. The index i denotes the quark flavour. Parton distributions are nonpertubative and have not be obtained from experiment.

\begin{equation}
{{F}_2}^{exp} (x)= \sum_i {e_i}^2 x f_i(x)
\end{equation}
The evolution of a parton
distribution due to splitting is called the DGLAP
...
\end{equation}
This is a system of coupled integral or differential equations. $ P_{ij}(\frac{x}{y}) $ represents the probability thar a daughter parton $ i $ with momentum fraction $ \frac{x}{y} $ splits from a parent parton $ j $.
...
\end{equation}
There four spiriting possibilities according to below diagrams and the transitions lead to ser of $ 2n_f +1 $ coupled evolution equations.
\begin{equation}
...
\end{equation}
A parton distribution changes when a different parton splits or the parton itself splits. we can not compute parton densities, but we
can predict how they evolve from one scale to another. we can measure PDFs in one process and use them as an input for another process.
