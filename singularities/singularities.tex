...
\begin{equation}
|A|^2 = |{A^{(0)}}_n|^2 +  |{A^{(0)}}_{n+1}|^2+ 2Re({A^{(0)*}}_{n}{A^{(1)}}_{n})
\end{equation}
Whereby, $ |{A^{(0)}}_n|^2 $ is the tree level contribution from Lo and has no divergences, $ |{A^{(0)}}_{n+1}|^2+ 2Re({A^{(0)*}}_{n}{A^{(1)}}_{n}) $ comes from the NLO and they are separately divergent. The problem in this case is that Integrations cannot be combined due to different phase space dimensions:

\begin{equation}
\sigma^{NLO} = \int_{n+1} \partial \sigma^R +\int_{n} \partial \sigma^V
\end{equation}
To tackle this problem one can use the subtraction method in that way one add and subtract the terms $ \partial \sigma^A $ to the integral. $ \partial \sigma^A $ approximates soft and collinear singularities of $ \partial \sigma^R  $.
\begin{equation}
\sigma^{NLO} = \int_{n+1} [\partial \sigma^R -\partial \sigma^A]+\int_{n} [\partial \sigma^V+\int_1 \partial \sigma^A]
\end{equation}

The virtual contribution must be UV-finite:
\begin{equation}
\int_{n} [\partial \sigma^V= \int_n [\int_{loop} \partial {\sigma^V}_{bare} + {\sigma^V}_{Counter\:term}]
\end{equation}
The addition of $\int_1 \partial \sigma^A$ to the $ \int_{n} [\partial \sigma^V $ ensures that IR poles are cancelled.
...
\begin{equation}
\int_{n} \partial \sigma^V +\int_{loop} \partial {\sigma^L}-\int_{loop} \partial {\sigma^L}= \int_n \int_{loop}[ \partial {\sigma^V}_{bare}- \partial {\sigma^L}] + \int_n[{\sigma^V}_{Counter\:term}+ \int_{loop} \partial {\sigma^L}]
\end{equation}


\begin{equation}
\sigma^{NLO} = \int_{n+1} [\partial \sigma^R -\partial \sigma^A]+ \int_n \int_{loop}[ \partial {\sigma^V}_{bare}- \partial {\sigma^L}] + \int_n[{\sigma^V}_{Counter\:term}+ \int_{loop} \partial {\sigma^L}+\partial \sigma^A]
\end{equation}
