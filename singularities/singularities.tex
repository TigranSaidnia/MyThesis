...
\frac{\alpha_s}{2\pi}\int_{x}^{1}\frac{dy}{y} f_j(\frac{x}{y}, \mu^2) P_{ij}(y)
\end{equation}
This is a system of coupled integro/differential equations.
The above convolution in compact notation in the general case:
\begin{equation}
\frac{\partial f(x, \mu^2) }{\partial \: ln \:\mu^2}=
\sum_j P_{ij} \otimes f_j(\mu^2)
\end{equation}
There three more spiriting possibilities according to below diagrams. A parton distribution changes when a different parton splits or the parton itself splits. we can not compute parton densities, but we
can predict how they evolve from one scale to another. we can measure pdfs in one process
