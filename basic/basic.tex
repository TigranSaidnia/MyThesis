...
were shown to be dynamical entities, instead of bookkeeping devices to classify the hadrons (Gell-Mann’s eightfold way).
...
\end{split}
\end{equation}
Now we can compute all possible States in regard to colour With Young Tableaux. One uses group theory methods, for instance the Young Tableaux technique, to decompose products of irreducible representations into sums.
\begin{figure}[h!]
\centering
...
\end{figure}
%https://web.physik.rwth-aachen.de/~hebbeker/lectures/sem0607/popenda_ausarbeitung.pdf
...
Here i,j are color indices in the fundamental representation, a  color index in the adjoint representation of SU(3) respectively. f labels the six flavours of quarks. $ g_s $ describes the strong coupling constant and $ {A^a}_{\mu} $ is the gluon field and it corresponds to anon-abelian gauge theory with structure constants $ f^{abc} $. $ {\chi^a} $ is a scalar field under Lorenz group, but anti commuting. With The field-strength tensor for QCD by:
\begin{equation}
\begin{split}
...
Fundamental representation in $ SU(3) $ are given by:
\begin{equation}
\begin{split}
